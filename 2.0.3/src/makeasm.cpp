...
std::string MakeDist(const parcel &in)
{
	...
	*/
	...
	// If building distribution rules for single file projects, a DDIR variable isn't generated,
	// so, this is a fix for it
	std::string OptDistDir = (IsMult) ? "" : AsString("DDIR = doc\n\n");
	ret += AsString(OptDistDir, DistLabels, DistAct, SelfLabels, SelfAct);
	return ret;
}
